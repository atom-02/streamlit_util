"""Streamlit web app: Markdown (+LaTeX) -> HWPX converter.

Deploy on Streamlit Community Cloud (share.streamlit.io) or run locally:
    pip install streamlit
    streamlit run streamlit_app.py

Pure Python: the only dependency is streamlit itself; the HWPX conversion uses
the standard library, and the base HWPX template is embedded below as base64.
"""
import base64, html, io, re, shutil, sys, tempfile, zipfile
...
# ===========================================================================
def convert_md_to_hwpx_bytes(md_text):
...
def _run_streamlit():
    ...
    st.set_page_config(page_title="Markdown → HWPX 변환기", page_icon="📄")
    st.title("📄 Markdown → HWPX 변환기")
    ...
    out_name = st.text_input("출력 파일 이름", value=f"{src_name}.hwpx")

    if st.button("변환하기", type="primary", disabled=(md_text is None)):
        if not md_text:
            st.warning("먼저 .md 파일을 올리거나 내용을 붙여넣으세요.")
            return
        name = out_name.strip() or "output.hwpx"
        if not name.lower().endswith(".hwpx"):
            name += ".hwpx"
        try:
            data, nb, ne = convert_md_to_hwpx_bytes(md_text)
...
# `streamlit run` executes the module top-to-bottom, so call the UI here.
# Running the plain file (python app.py) prints a hint instead of crashing.
try:
...
if _HAS_ST:
    ...
    print("이 파일은 Streamlit 앱입니다. 다음처럼 실행하세요:\n"
          "    pip install streamlit\n"
          "    streamlit run streamlit_app.py")
